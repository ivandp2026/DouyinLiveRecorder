...
开启分段录制后，每个 30 分钟视频片段都有独立的弹幕、时间线、关键词、热点和 HTML 报告，并放在自己的文件夹中：
...
├── 主播_时间_000/
│   ├── 主播_时间_000.ts
│   ├── 主播_时间_000.danmaku.mp4
│   ├── 主播_时间_000.danmaku.raw.jsonl
│   ├── 主播_时间_000.danmaku.timeline.csv
│   ├── 主播_时间_000.danmaku.keywords.json
│   ├── 主播_时间_000.highlights.json
│   └── 主播_时间_000.danmaku.report.html
└── 主播_时间_001/
    └── 同一套属于第 2 段的文件
```

程序会读取每个原始片段的实际媒体时长，把该时间窗口内的弹幕和统计切出来，并将时间重置为 `00:00:00`。因此各段数据不会互相混入，即使切片点因关键帧不是精确的 1800 秒也不会累积偏差。`segments.json` 汇总全部分段、总时长和文件路径。
